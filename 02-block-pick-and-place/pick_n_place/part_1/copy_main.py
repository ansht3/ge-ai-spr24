...
def main(sim_path, horizon):
    ...
    while True:
        ...
        if sim.data.time == 0:
	        ...
	        eef_mat = sim.data.xmat[eef_sid]  # identification number of hand to get the returns the rotation matrix to figure out orientation

	        target_pos = sim.data.xpos[target_sid]
	        target_mat = sim.data.xpos[target_sid]



	        # able to help determine the system properly into robot understandable commands
	        start_pos = sim.data.xpos[start_sid]
            start_mat = sim.data.xpos[start_sid]
	        ...
	        # creating scond ik result 

    	    ik_result_2 = qpos_from_site_pos(
		        physics=sim,
		        site_name="end_effector",
    		    target_pos=box_pos,
	        ...
	        waypoints2 = generate_join_space_min_jerk(
		        start=ik_result_2.qpos[:ARM_nJnt],
		        goal=ik_result_2.qpos[:ARM_nJnt],
		        time_to_go=horizon,
		        dt = sim.model.opt.timestep,
 	        )
	        ik_result3 = qpos_from_site_pose(
 		        physics=sim,
		        site_name = "end_effector",
		        target_pos = box_pos + np.array([0, 0, 0.5]),
		        target_quat = np.array([0, 1, 0, 0]),
		        inplace=False,
		        regularization_strength=1.0,
	        )   
		...
		physics=sim,
		site_name="end_effector",
		targt_pos = target_pos + np.array([0, 0, 0.5])
